...
// Cover-art slot on the board (SPEC §4.4): square, 172px, pushed as RGB565.
// CONTRACT PAIR: this MUST match the board's COVER_PX (firmware/main/net_backend.c).
// The board rejects any cover whose header dimensions != its expected size, so
// these two constants must always be bumped together (SPEC-ytmusic-now-playing.md).
export const COVER_PX = Number(process.env.COVER_PX ?? 172);
