...
{
    np_modes_t m;
    m.disc      = vm->conn_state == CONN_OFFLINE || !vm->host_connected;
...
void now_playing_update(const now_playing_vm_t *vm)
{
    ...
    // Leaving the loader (s_force_full) rebuilds everything, like the first run:
    // the overlay may have hidden state the diff below would otherwise skip.
    bool first = !s_have_last || s_force_full;
    s_force_full = false;
